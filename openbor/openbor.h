...
typedef struct
{
	...
	unsigned long   pScores[MAX_PLAYERS];			// Player Scores
...
typedef struct
{
    ...
    int             staydown;                       // Add to risetime.
    int             staydownatk;                    // Add delay before riseattack.
}s_attack;
...
typedef struct
{
	...
	char			mponly:8;						// Does energy get taken from magic bar, lifebar or both?
    char			fastattack:8;					// Flag to determine if the opponent uses their pain time
    short			energycost:16;					// 1-10-05 to adjust the amount of energy used for specials
    ...
    int				range[6];						// Use for attacks, xmin, xmax, zmin, zmax, amin, amax
    ...
    short			counterframe:16;				// frame in which a counter is possible
    char			countercond:8;					// conditions to switch to a FOLLOW anim
    char			counterdamage:8;				// You can specify if counterattackers take damage
    char            cancel:8;                       // Cancel anims with freespecial
	short*			weaponframe;					// Specify with a frame when to switch to a weapon model
	short*			quakeframe;						// Specify with a frame, repeat, quake (4 is highest)
...
typedef struct
{
    ...
    char			holdblock:8;
    char            nopassiveblock:8;               // dont auto blcock randomly
    ...
    char			weapnum:8;
    char			secret:8;
	char			weaploss:8;						// Determines possibility of losing weapon.
	...
	short			risetime:16;					// For enemy A.I.
	short			sleepwait:16;
	char            riseattacktype:8;
	unsigned long   jugglepoints[2];                // juggle points. [0] = current [1] = max total
	unsigned long   guardpoints[2];                 // guad points. [0] = current [1] = max total
	...
	short           stealth:16;                     // Entity's invisibility to AI.
	short           detect:16;                      // Entity's ability to "see" another entity with stealth.
	...
	unsigned long   aimove:32;                      // move style
	unsigned long   aiattack:32;                    // attack/defend style
...
typedef struct entity
{
    ...
    unsigned long	maptime;						// used by forcemap
    unsigned long	sealtime;						// used by seal (stops special moves).
    unsigned long   dot_time[MAX_DOTS]; 			//Dot time to expire.
    ...
    unsigned long	magictime;
	unsigned long   guardtime;
    unsigned long	nextanim;
    unsigned long	nextthink;
    unsigned long	pain_time;
    unsigned long   mpchargetime;					// For the CHARGE animation
    unsigned long   sleeptime;						// For the SLEEP animation
	unsigned long	rushtime;						// rush combo timer
	unsigned long   knockdowntime;                  // count knock down hit
	unsigned long	invinctime;						// Used to set time for invincibility to expire
	unsigned long   turntime;
    unsigned long   staydown;                       // Time before next rise modifier.
    unsigned long   staydownatk;                    // Extra time before riseattack, or -1 to disable riseattack.
    // -------------------------end of times ------------------------------
    char            update_mark:8;
...
typedef struct
{
    ...
    int			    mp;							// mp's variable for mpbar by tails
    unsigned long	score:32;					// So score can be overridden for enemies/obstacles
...
typedef struct
{
	...
	int             bosses;
    char			bossmusic[256];
	unsigned long	bossmusic_offset;
    int             numpalettes;
