...
void drawScreens(SDL_Surface *Image)
{
	...
	{
		case 2:
			FirstPass = SDL_AllocSurface(SDL_SWSURFACE, Screen->w,Screen->h, 32, 0, 0, 0, 0);
			(*GfxBlitters[BLITTER_MAX + BLITTER_HQ2X])((u8*)Scaler->pixels+Scaler->pitch*4+4, Scaler->pitch, pDeltaBuffer+Scaler->pitch, (u8*)FirstPass->pixels, FirstPass->pitch, FirstPass->w>>1, FirstPass->h>>1);
		...
		case 4:
			FirstPass = SDL_AllocSurface(SDL_SWSURFACE, Screen->w, Screen->h, 32, 0, 0, 0, 0);
			(*GfxBlitters[BLITTER_MAX + BLITTER_HQ2X])((u8*)Scaler->pixels+Scaler->pitch*4+4, Scaler->pitch, pDeltaBuffer+Scaler->pitch, (u8*)FirstPass->pixels, FirstPass->pitch, FirstPass->w>>1, FirstPass->h>>1);
			SecondPass = SDL_AllocSurface(SDL_SWSURFACE, FirstPass->w, FirstPass->h, 32, 0, 0, 0, 0);
			(*GfxBlitters[BLITTER_MAX + BLITTER_HQ2X])((u8*)FirstPass->pixels+FirstPass->pitch, FirstPass->pitch, pDeltaBuffer+FirstPass->pitch, (u8*)SecondPass->pixels, SecondPass->pitch, SecondPass->w>>1, SecondPass->h>>1);
...
void printText(int x, int y, int col, int backcol, int fill, char *format, ...)
{
	...
	for(i=0; i<sizeof(buf); i++)
	{
		...
		// draw
		//line = (unsigned long *)Scaler->pixels + x + y * Scaler->w;
		line = (unsigned short *)Scaler->pixels + x + y * Scaler->w;
		for(y1=0; y1<10; y1++)
		{
			data = *font++;
			for(x1=0; x1<5; x1++)
...
SDL_Surface *getPreview(char *filename)
{
	...
	title = loadscreen("data/bgs/title.gif", packfile, realPal, PIXEL_8);
	if(title == NULL) return NULL;
	scale = allocscreen(width, height, title->pixelformat);
	if(scale == NULL) return NULL;
	scalescreen(scale, title);
	image = SDL_AllocSurface(SDL_SWSURFACE, width, height, 8, 0, 0, 0, 0);
	...
	if(title != NULL) {freescreen(title); title=NULL;}
	if(scale != NULL) {freescreen(scale); scale=NULL;}
...
void initMenu(int type)
{
	...
	if(!type) Source = IMG_ReadXPMFromArray(isWide ? OpenBOR_Logo_480x272 : OpenBOR_Logo_320x240);
	else Source = IMG_ReadXPMFromArray(isWide ? OpenBOR_Menu_480x272 : OpenBOR_Menu_320x240);


	// Depending on which mode we are in (WideScreen/FullScreen)
	// allocate proper size for SDL_Surface to perform final Blitting.
	/*Edited for Dingoo
	Screen = SDL_SetVideoMode(Source->w * factor, Source->h * factor, 32, flags);*/

	Screen = SDL_SetVideoMode(Source->w * factor, Source->h * factor, 16, flags);
...
void drawBGMPlayer()
{
	...
	Image = SDL_AllocSurface(SDL_SWSURFACE, Source->w, Source->h, 32, 0, 0, 0, 0);
	if (Image != NULL)
	{
...
void drawLogs()
{
    ...
    Viewer = SDL_AllocSurface(SDL_SWSURFACE, Source->w, Source->h, 32, 0, 0, 0, 0);
    bothkeys = bothnewkeys = 0;
...
//DEBUG
	...
	if((dListTotal = findPaks()) != 1)
	{
		...
		while(!done)
		{
//fprintf(stderr,"dentro while ! done\n");
			sound_update_music();
        ...
        freeAllLogs();
		termMenu();
        if(ctrl == 2) { tracefree(filelist); borExit(0); }
	}
