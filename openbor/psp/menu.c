...
Image *getPreview(char *filename)
{
	...
	title = loadscreen("data/bgs/title.gif", packfile, pal, pixelformat);
	if(title == NULL) return NULL;
	scaledown = allocscreen(width, height, pixelformat==PIXEL_x8?PIXEL_32:PIXEL_8);
	if(scaledown == NULL) return NULL;
