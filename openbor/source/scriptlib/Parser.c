...
*  Returns:
******************************************************************************/
void Parser_ParseText(Parser* pparser, List* pIList, LPCSTR scriptText,
                     ULONG startingLineNumber, LPCSTR path )
{
   ...
   Lexer_Init(&(pparser->theLexer), scriptText, thePosition );
...
*  Returns:
******************************************************************************/
void Parser_ParseExpression(Parser* pparser, List* pIList, LPCSTR scriptText,
                     ULONG startingLineNumber, LPCSTR path )
{
   TEXTPOS thePosition;
   ...
   LPCSTR expressionText = (CHAR*)tracemalloc("Parser_ParseExpression", sizeof(CHAR) * strlen(scriptText) + 2);
   ...
   pparser->errorFound = FALSE;
   Lexer_Init(&(pparser->theLexer), expressionText, thePosition );
...
Label Parser_CreateLabel( Parser* pparser )
{
    ...
    sprintf((CHAR*)theLabel, "L%ld", pparser->LabelCount);
...
{
    if (ParserSet_First(&(pparser->theParserSet), log_or_expr, pparser->theNextToken.theType )){//= /= +=之类运算符或者逗号（下一个变量声名）
...
{
	char buf[64];
    //Report the offending token to the error handler, 
	//along with the production it offended in.
    //HandleCompileError( m_pNextToken, (ERRORVAL)offender );
    strncpy(buf, pparser->theLexer.pcurChar, 63);
    printf("\nParser Error %u found in scripts '%s', line %d, column %d, %s '%s'.\n", offender, pparser->currentPath, pparser->theLexer.theTokenPosition.row, pparser->theLexer.theTokenPosition.col, 
     _production_error_message(pparser, offender), pparser->theNextToken.theSource);
    printf("Before here:\n%s\n", buf);
    ...
    while ((pparser->theNextToken.theType != TOKEN_EOF) && 
          (!ParserSet_Follow(&(pparser->theParserSet), offender, pparser->theNextToken.theType)))
		  Parser_Match(pparser);
