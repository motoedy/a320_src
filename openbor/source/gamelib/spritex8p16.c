...
static unsigned short fillcolor = 0;
...
static void scaleline(int x, int cx, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp,  int scale)
{
    unsigned char* data;
    int dx, i, d;
...
static void scaleline_flip(int x, int cx, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp,  int scale)
{
    unsigned char* data;
    int dx, i, d;
    unsigned char * charptr;
...
static void scaleline_rot(int x, int y, int cx, int dy, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp, int scale, int angle)
{
    unsigned char* data;
    int dx, rx, ry, i, d;
    unsigned char * charptr;
...
// x: centerx on screen; cx: centerx of this line; y: centery on screen; dy: the line's y coordination
// angle: rotate angle, should be a positive value from 0 - 359, check it before use this function
static void scaleline_rotflip(int x, int y, int cx, int dy, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp,  int scale, int angle)
{
    unsigned char* data;
    int dx, rx, ry, i, d;
    unsigned char * charptr;
...
void putsprite_ex_x8p16(int x, int y, s_sprite *frame, s_screen *screen, s_drawmethod* drawmethod)
{
	...
	unsigned short *dest_c;
	int scale=0, old_scale=0;
	void (*scalelinefp)(int x, int cx, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp,  int scale);
    void (*scalelinerotfp)(int x, int y, int cx, int dy, int width, int *linetab, unsigned short* palette, unsigned short *dest_c, blend16fp fp,  int scale, int angle);
    ...
    if(drawmethod->alpha>0) drawmethod->fp = blendfunctions16[drawmethod->alpha-1];
        ...
        if(drawmethod->flipy)
        {
            ...
            while(height--)
            {
                ...
                if(d >= 256) // > 1pixel, so draw these
                {
                    ...
                    {
                        scalelinerotfp(x+((drawmethod->shiftx*(cy-y))/256), cy, centerx, y, frame->width, linetab, (unsigned short*)(drawmethod->table?drawmethod->table:frame->palette), (unsigned short*)(screen->data), (blend16fp)drawmethod->fp, drawmethod->scalex, drawmethod->rotate);
        ...
        else // un-flipped version
        {
            ...
            while(height--)
            {
                ...
                if(d >= 256) // > 1pixel, so draw these
                {
                    for(i=d>>8; i>0; i--) // draw a line
                    {
                        scalelinerotfp(x+((drawmethod->shiftx*(y-cy))/256), cy, centerx, y, frame->width, linetab, (unsigned short*)(drawmethod->table?drawmethod->table:frame->palette), (unsigned short*)(screen->data), (blend16fp)drawmethod->fp, drawmethod->scalex, drawmethod->rotate);
                        ++y;
                    }
                    old_scale = scale & 0xFFFFFF00; //truncate those less than 256
                }
...
//----------------------------------------------------------------------------------------------------------
    {
        ...
        if(drawmethod->flipy)
        {
            ...
            if(y<0) return;
            scale -= t<0?((-t)%256):(t%256);
            dest_c = (unsigned short*)(screen->data)+y*screenwidth;
            while(height--)
            {
                scale += drawmethod->scaley;
                d = scale - old_scale; // count scale added
                if(d >= 256) // > 1pixel, so draw these
                {
                    for(i=d>>8; i>0; i--) // draw a line
                    {
                        ...
                        {
                            scalelinefp(x+((drawmethod->shiftx*(cy-y))/256), centerx, frame->width, linetab, (unsigned short*)(drawmethod->table?drawmethod->table:frame->palette), dest_c, (blend16fp)drawmethod->fp, drawmethod->scalex);
                        }
            ...
            while(height--)
            {
                ...
                if(d >= 256) // > 1pixel, so draw these
                {
                    for(i=d>>8; i>0; i--) // draw a line
                    {
                        ...
                        {
                            scalelinefp(x+((drawmethod->shiftx*(y-cy))/256), centerx, frame->width, linetab, (unsigned short*)(drawmethod->table?drawmethod->table:frame->palette), dest_c, (blend16fp)drawmethod->fp, drawmethod->scalex);
                        }
