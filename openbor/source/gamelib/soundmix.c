...
**
**	Note:
**  If any of the #defines are increase in size
**  pay close attention to the the size of variables
**  which are used in conjuntion with.  As you could
**  be going beyond the variables current size which
**  will cause errors!!!
*/
...
void update_sample(unsigned char *buf, int size) {
	...
	if (playbits==8) {
		...
		for(i=0;i<todo;i++) {
			...
			if (u<0) u=0; else if (u>0xff) u = 0xff;
			dst[i] = u;
		}
	}
	else {
		...
		for(i=0;i<todo;i++) {
			u = mixbuf[i] >> MIXSHIFT;
			if (u<0) u=0; else if (u>0xffff) u = 0xffff;
#ifndef LINUX
			u^=0x8000;
#endif
			dst[i] = u;
...
static unsigned long loop_offset;
...
int sound_start_playback(int bits, int frequency){
...
#else
    // Consoles only support 16/44100
    bits = 16;
    frequency = 44100;
	playbits = bits;
	playfrequency = frequency;
	...
	for(i=0; i<max_channels; i++) sound_stop_sample(i);
	SB_playstop();
	if(!SB_playstart(playbits, playfrequency)) return 0;
