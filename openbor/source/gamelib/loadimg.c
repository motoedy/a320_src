...
static int openpng(char *filename, char *packfilename)
{
    ...
    if((handle=openpackfile(filename,packfilename))==-1) goto openpng_abort;

    if(readpackfile(handle, header, 8)!=8) goto openpng_abort;
    ...
    current_res[0] = info_ptr->width;
    png_height = current_res[1] = info_ptr->height;
...
static int readpng(unsigned char *buf, unsigned char *pal, int maxwidth, int maxheight)
{
    ...
    cw = current_res[0]>maxwidth?maxwidth:current_res[0];
    ch = current_res[1]>maxheight?maxheight:current_res[1];
    if(buf)
    {
        for(i=0; i<ch; i++)
        {
...
static int readgif(unsigned char *buf, unsigned char *pal, int maxwidth, int maxheight){
    ...
    while(!done){
        ...
        switch(c){
            ...
            if(readpackfile(handle,&c,1)!=1) return 0;
            if(c<2 || c>8) return 0;
            if(!decodegifblock(handle, buf, maxwidth, maxheight, c, &iblock)) return 0;
            break;
            case '!':
        ...
        while(y<=pcx_header.ymax && y<maxheight){
            ...
            while(cpos<63990 && y<=pcx_header.ymax && y<maxheight){
                if(codebuffer[cpos]>192){
                    ...
                    do{
                        if(x<maxwidth) buf[dpos] = (signed char)repbyte;
                        ++dpos;
                    ...
                    if(x<maxwidth) buf[dpos] = (signed char)codebuffer[cpos];
                    ++cpos;
                    ++dpos;
                    if((++x)>=pcx_header.bytesperline){
                        x=0;
...
s_screen * loadscreen(char *filename, char *packfile, unsigned char *pal, int format){
    ...
    screen = allocscreen(current_res[0], current_res[1], format);
    if(screen == NULL){
        closeimage();
        return NULL;
    }
