...
void putscreenx8p32_water(s_screen * dest, s_screen * src, int x, int y, int key, unsigned* remap, unsigned (*blendfp)(unsigned,unsigned), int amplitude, float wavelength, int time, int watermode)
{
	...
	if(!remap) remap = (unsigned*)src->palette;
